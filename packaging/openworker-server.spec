...
# coworker's package DATA — `personas/builtin/*.md` (declared in pyproject's package-data).
# collect_submodules only walks importable modules, so without this the markdown-backed
# personas are absent from the bundle and the desktop app silently ships with only the three
...
# [knowledge] extras — Office deliverables, image editing, and the analysis kernel.
# All of these are lazy-imported behind `_common.require()` so PyInstaller's static analysis
# never sees them; without collect_all they are absent from the bundle and every Office /
# analysis tool fails at runtime with "not installed" on an app the user cannot pip into.
#
# They are also DATA-heavy, which is why collect_all rather than collect_submodules:
#   docx/pptx  ship the default .docx/.pptx templates they build new documents from
...
a = Analysis(
    ...
    hookspath=[],
    runtime_hooks=[],
    # matplotlib and PIL used to be excluded (nothing needed them, and they are large). The
    # knowledge-work toolset needs both — matplotlib renders analysis charts, PIL backs every
    # image tool — so they are bundled now. Only GUI toolkits stay excluded: they would pull
    # Qt/Tk into a headless sidecar for nothing. matplotlib is forced to the Agg backend at
    # kernel start (see tools/analysis/kernel.py), so it never looks for one.
