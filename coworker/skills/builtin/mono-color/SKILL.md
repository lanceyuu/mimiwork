...
Save generated files into the current working folder (in MimiWork, the session's workspace). Stop at prompt-only only when the user explicitly asks for it or no image-generation capability is available.
