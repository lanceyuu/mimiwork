...
class PersonaRegistry:
    ...
    def bound_model(self, persona_id: str) -> str:
        """The model this persona pins ("provider:model-id"), or "" when it leaves the choice
        to the session. Only manifest-backed personas can bind one."""
        entry = self._entries.get(persona_id)
        if entry is None or entry.manifest is None:
            return ""
        return getattr(entry.manifest, "model", "") or ""
