...
const PERSONAS = {
  personas: [
    { id: "cowork", name: "MimiWork", icon: "cowork", tagline: "Produce a deliverable — research, analysis, scripts", needs_workspace: true, builtin: true, family: "knowledge", workspace: "deliverable", tools: ["files", "search"], enabled: true, surfaced: true, default: true },
...
const PINNED_SESSION = {
  ...
  workspace: "/Users/test/MimiWork/launch-note",
  agent: "cowork",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const OPS_SESSION = {
  session_id: "ops-1",
  title: "Ops triage",
  workspace: "/Users/test/MimiWork/ops-triage",
  agent: "ops",
  model: "anthropic:claude-opus-4-8",
  mode: "interactive",
...
const CONNECTORS = {
  connectors: [
    ...
    // email: the tallest setup form (four instructions, seven fields) — the modal must
    // scroll on a laptop screen (owner hit 2026-09-09).
    { name: "email", title: "Email (IMAP)", icon: "✉", blurb: "Read, search, and send mail from any IMAP account — Gmail, iCloud, Fastmail, or custom.", auth: "api_token", two_way: false, channels: false, available: true, brand_color: "#2563eb", logo: "email", fields: [{ key: "address", label: "Email address", secret: false, required: true, help: "", placeholder: "you@gmail.com" }, { key: "app_password", label: "App password", secret: true, required: true, help: "Gmail/iCloud: generate an app password (requires 2-step verification). Not your account password.", placeholder: "" }, { key: "display_name", label: "Display name", secret: false, required: false, help: "Shown as the From name on sent mail.", placeholder: "" }, { key: "imap_host", label: "IMAP host (advanced)", secret: false, required: false, help: "Only needed for providers we don't auto-detect.", placeholder: "imap.example.com" }, { key: "imap_port", label: "IMAP port (advanced)", secret: false, required: false, help: "", placeholder: "993" }, { key: "smtp_host", label: "SMTP host (advanced)", secret: false, required: false, help: "", placeholder: "smtp.example.com" }, { key: "smtp_port", label: "SMTP port (advanced)", secret: false, required: false, help: "", placeholder: "587" }], instructions: ["Gmail: turn on 2-Step Verification, then create an app password at myaccount.google.com/apppasswords.", "iCloud: generate an app-specific password at account.apple.com → Sign-In and Security.", "Enter your address and the app password below. Gmail, iCloud, and Fastmail servers are detected automatically; for other providers fill in the IMAP/SMTP hosts.", "Note: Google Workspace and Microsoft 365 accounts often have IMAP or app passwords disabled by the org admin."], connected: false, account: null, enabled: false, allowed_users: [], tools: [], managed: false, managed_profile: false },
...
const PRIMARY_ROOT = { path: "/Users/test/MimiWork/launch-note", writable: true, label: "scratch", primary: true, exists: true };
...
const PROVIDERS = [
  ...
  // openrouter: configured, with a LIVE catalog of free models (the server feeds it from
  // openrouter.ai) — long enough that the checklist folds to a shortlist.
  { name: "openrouter", title: "OpenRouter", needs_key: true, fields: [{ key: "api_key", label: "OpenRouter API key", secret: true, required: true, help: "", placeholder: "sk-or-…" }], configured: true, values: {}, suggested_models: ["minimax/minimax-m3:free", "nvidia/nemotron-3-ultra-550b-a55b:free", "nvidia/nemotron-3.5-lightning:free", "thinkingmachines/inkling:free", "dots-studio/dots-3-note-preview:free", "google/gemma-4-31b-it:free", "google/gemma-4-26b-a4b-it:free", "inclusionai/ling-3.0-flash-sante:free", "poolside/laguna-s-2.1:free", "cohere/north-mini-code:free", "nvidia/nemotron-3-super-120b-a12b:free", "minimax/minimax-m2.7:free", "liquid/lfm-2.5-2.6b:free"], key_set_at: "2026-09-06", last_used_at: null },
