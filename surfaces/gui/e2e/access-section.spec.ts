...
test("+ Add a source: full catalog on focus, filter as you type → connect-in-context; connected sources never match", async ({
  ...
  await expect(rows).toHaveCount(10); // 13 in the catalog minus browser/slack/github (connected)
  ...
  const body = page.getByRole("region", { name: "Session access" });
  await expect(body.getByText("Connecting makes Notion available in every conversation", { exact: false })).toBeVisible();
